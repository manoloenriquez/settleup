# CODEX.md — Agent Context for Codex
...
1. **Read the target files before editing** — understand existing code first
2. **Make one logical change per edit** — keep diffs focused and reviewable
3. **Show exact file paths and line ranges** in your response
4. **After edits, provide test commands** to verify the change
5. **Never guess at types** — read `database.types.ts` or schema files
...
```bash
pnpm --filter @template/shared test    # Vitest — shared utils/schemas
pnpm --filter @template/web build      # Next.js production build
pnpm typecheck                         # TypeScript across all packages
pnpm lint                              # ESLint across all packages
```

Test files: `packages/shared/src/__tests__/` — `money.test.ts`, `split.test.ts`, `parser.test.ts`, `debts.test.ts`
