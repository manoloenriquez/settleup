...
import { generateJSON, generateJSONFromImage } from "../core/generate";
 ...
 * When `strict` is true, the regex fallback is skipped and an error is returned instead
 * (used by mobile, which doesn't want regex results).
 */
export async function parseReceiptImage(
  ...
  // Step 3: Regex fallback on OCR text (skipped in strict mode)
  if (options.strict) {
    return { data: null, error: "Could not parse receipt with AI. Please try again." };
  }
  const rawText = await extractTextWithOCR(buffer);
  if (rawText) {
