-- Fix inverted payment signs in get_friend_view and get_group_overview
-- Both RPCs had + received - sent; correct formula is - received + sent
-- (matches get_member_balances and get_groups_with_stats)
...
BEGIN
  ...
  -- Expense items for this member, with per-expense line item breakdown
...
-- Fix get_group_overview payment signs
...
BEGIN
  ...
  -- 4-source balance per member: paid_as_payer - shares - received_payments + sent_payments
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'member_id',    gm.id,
  ...
  -- All expenses with per-participant breakdown and line items
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'item_name',    e.item_name,
