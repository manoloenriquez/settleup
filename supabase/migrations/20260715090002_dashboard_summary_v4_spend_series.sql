-- Dashboard summary v4 (superset): copies the v3 body verbatim from
-- 20260713185452_dashboard_summary_v3_superset.sql and adds ONE key,
-- `spend_series` — the signed-in user's own daily expense share
-- (SUM of their claimed members' share_cents) over the last 30 days across
-- accessible non-archived groups. This backs a real dashboard sparkline
-- (replacing the decorative hardcoded SVG). Requires expenses.expense_date
-- (20260715090000_expense_date.sql).
--
-- Older clients ignore the extra key; newer clients default spend_series to
-- [] when talking to an older DB (see dashboardSummarySchema).
