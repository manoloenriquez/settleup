-- Fix payment signs in all three balance RPCs.
-- Correct formula: net = paid_as_payer - shares - received_payments + sent_payments
-- Receiving a payment reduces what the payer is owed (subtract).
-- Sending a payment reduces what the sender owes (add back).
