-- =============================================================================
-- Phase 2.4: Mask sensitive payment profile data in public (anon) RPCs
--
-- get_friend_view and get_group_overview are callable without authentication.
-- Bank account numbers and GCash numbers are partially masked so that
-- anonymous viewers cannot harvest full financial details.
--
-- Masking rule: show only the last 4 characters preceded by "****".
-- Names and QR URLs remain unmasked (they are non-secret display values).
-- =============================================================================

-- Helper: mask all but the last 4 characters of a string
CREATE OR REPLACE FUNCTION settleup.mask_account(p_value TEXT)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN p_value IS NULL OR length(p_value) <= 4 THEN p_value
    ELSE '****' || right(p_value, 4)
  END;
$$;

-- ---------------------------------------------------------------------------
-- Update get_friend_view to mask sensitive fields
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Payment profile — mask account numbers
...
-- ---------------------------------------------------------------------------
-- Update get_group_overview to mask sensitive fields
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- All expenses with participant breakdown
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'item_name',    e.item_name,
