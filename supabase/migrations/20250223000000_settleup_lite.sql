...
CREATE POLICY "group_members_owner_all"
  ...
  TO authenticated
  USING (group_id IN (SELECT id FROM public.groups WHERE owner_user_id = auth.uid()))
  WITH CHECK (group_id IN (SELECT id FROM public.groups WHERE owner_user_id = auth.uid()));
...
CREATE OR REPLACE FUNCTION public.get_friend_view(p_share_token TEXT)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.get_friend_view(TEXT) TO anon;
GRANT EXECUTE ON FUNCTION public.get_friend_view(TEXT) TO authenticated;

-- ---------------------------------------------------------------------------
-- Storage bucket: payment-qr (public)
-- ---------------------------------------------------------------------------
...
-- Authenticated users can delete their own QR images
CREATE POLICY "payment_qr_delete"
  ON storage.objects FOR DELETE
  TO authenticated
  USING (bucket_id = 'payment-qr');

-- Public can read QR images
