-- =============================================================================
-- Update get_friend_view and get_group_overview to return creditor-specific
-- payment profiles instead of always showing the group owner's profile.
--
-- New fields:
--   all_balances      — all members' net_cents (for client-side simplifyDebts)
--   creditor_profiles — payment profiles for members with positive net_cents
--   payment_profile   — kept for backward compat (owner's profile, masked)
-- =============================================================================

-- ---------------------------------------------------------------------------
-- Updated get_friend_view
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- 4-source balance for this member
  SELECT (
  ...
  -- All member balances (for client-side simplifyDebts)
  ...
  -- Payment profiles for creditors (members with positive net_cents, masked for anon)
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'member_id',           gm.id,
      'display_name',        gm.display_name,
  ...
  -- Owner payment profile (backward compat, masked)
...
BEGIN
  ...
  -- Creditor payment profiles (masked for anon)
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'member_id',           gm.id,
      'display_name',        gm.display_name,
