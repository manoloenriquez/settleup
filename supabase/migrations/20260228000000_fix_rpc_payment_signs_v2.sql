-- Fix payment sign convention in all 3 RPCs.
--
-- Previous migrations (20260225000003, 20260225000004) reintroduced the wrong
-- formula after 20260224000006 had corrected it.
--
-- Correct formula:
--   net = paid_as_payer - shares - received_payments + sent_payments
--
-- Rationale:
--   positive net → member is owed money (paid more than their share)
--   negative net → member owes money
--   Receiving a payment (to_member_id) settles what you're owed → subtract
--   Sending a payment (from_member_id) settles what you owe   → add back
...
BEGIN
  ...
  -- 4-source balance per member: paid - shares - received + sent
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'member_id',    gm.id,
      'display_name', gm.display_name,
